...
│   Bias-T DC提取 → 1N4007防反 → LM2940CT-12(12V) → LM2596模块(6V) → AMS1117-3.3(3.3V)
│
├── Sheet 2/4: MCU (ESP32-S3 控制)
│   ESP32-S3-WROOM-1 + USB Serial/JTAG + Bias-V ADC + 伺服接口 + 指示
│
├── Sheet 3/4: HV_TANK (高压谐振回路)
│   T200-6 单磁芯 2:14 耦合器 + 空气可变电容 10-500pF (伺服驱动)
│
...
同轴电缆芯线同时承载 RF 和 13.8V DC。L_bias 扼流圈分离 DC，经 1N4007 防反接后送入 LM2940CT-12 稳压到 12V。12V 轨输入 LM2596 DC-DC 模块降压为 6V 供伺服电机。AMS1117-3.3 LDO 将 12V 降压为 3.3V 供 ESP32-S3。
...
| GPIO2 | SERVO_PWR_CUT | OUT | 2N2222A Base (经1kΩ) → Q1 Gate控制 | HIGH=伺服断电 |
...
```
SO-239 芯线 → 10nF×2 隔直 → T200-6 初级2匝热端
T200-6 初级2匝冷端 → SO-239 外壳 (GND)
T200-6 次级14匝热端 → 可变电容定片 + ANT 端子
T200-6 次级14匝冷端 → GND + 可变电机动片
可变电容动片 → GND
```

**关键**: 所有 RF 高压路径采用 1.5mm² 以上铜导线直接连接，不经过 PCB 走线。T200-6 磁环用尼龙扎带固定在底板上。可变电容和伺服电机安装在铝壳底板上，通过齿轮组耦合。
